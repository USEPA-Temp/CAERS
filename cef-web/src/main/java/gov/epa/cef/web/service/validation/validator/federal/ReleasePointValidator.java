...
public class ReleasePointValidator extends BaseValidator<ReleasePoint> {
    ...
    private static final String DIAMETER_FORMULA = ": Exit Gas Velocity = Flow Rate / (Pi * (Stack Diameter /2) ^ 2) for circular stacks.";
    private static final String LENGTH_WIDTH_FORMULA = ": Exit Gas Velocity = Flow Rate / (Stack Length * Stack Width) for rectangular stacks.";
        ...
        if (ConstantUtils.STATUS_OPERATING.contentEquals(releasePoint.getOperatingStatusCode().getCode())) {
            ...
            if (!releasePoint.getTypeCode().getCode().equals(FUGITIVE_RELEASE_POINT_CODE)) {
                ...
                if ((releasePoint.getExitGasFlowRate() != null && releasePoint.getExitGasFlowRate() > 0)
                    ...
                    String stackMeasurement = isDiameter ? "Diameter is" : "Stack Length/Stack Width are";
