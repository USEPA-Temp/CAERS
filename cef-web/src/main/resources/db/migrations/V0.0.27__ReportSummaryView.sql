...
SELECT CONCAT(fs.id, er.year) AS id, e.pollutant_cas_id AS pollutant_cas_id, e.pollutant_name AS pollutant_name, e.pollutant_type AS pollutant_type, 
    fugitive_amount.calculated_emissions_tons AS fugitive_total, stack_amount.calculated_emissions_tons AS stack_total, 
...
FROM emission e
    ...
    INNER JOIN (SELECT sum(e.calculated_emissions_tons) AS calculated_emissions_tons, er.year, fs.id AS facility_id, e.pollutant_cas_id
                FROM emission e
                    INNER JOIN reporting_period repPer ON repPer.id = e.reporting_period_id
                    INNER JOIN emissions_process ep ON ep.id = repPer.emissions_process_id
                    INNER JOIN release_point_appt rpa ON ep.id = rpa.emissions_process_id
